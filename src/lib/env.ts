...
export function getEnv(): EnvVars {
  if (typeof window !== 'undefined') {
    ...
    const env = {
      ...
      [ENV_KEYS.SYSTEM_PROMPT]: window.__env__?.[ENV_KEYS.SYSTEM_PROMPT] || import.meta.env[ENV_KEYS.SYSTEM_PROMPT]
    };
    validateEnvVars(env);
    return env;
  } else {
    ...
    const env = {
      ...
      [ENV_KEYS.SYSTEM_PROMPT]: process.env[ENV_KEYS.SYSTEM_PROMPT]
    };
    validateEnvVars(env);
    return env;
  }
